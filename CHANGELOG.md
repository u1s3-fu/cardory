...
- 附件关联资产：数据库 schema v3 为附件表新增 `asset_id` 列（附迁移测试），资产详情新增「关联附件」区（展示、解除关联），项目附件面板支持把附件关联到同项目资产；关联关系随实体级增量同步传播，附件正文与云端清单不受影响。
- 资产类型模板化：资产类型从固定「软件/硬件」二选一升级为模板驱动——内置软件、硬件、域名、SSL 证书四套模板（字段含文本/数值/日期/链接，日期字段可标记到期提醒），设置页可启用/禁用内置模板，登记表单与详情按模板动态渲染；存量软件/硬件资产读取时自动归一化挂载内置模板，模板与自定义字段存于资产元数据（旧字段键保留，旧版本读盘兼容），模板清单随配置文档同步；账号/密码仍为所有模板的公共字段，继续走敏感列且不进入同步载荷。
- 资产到期日接入日历：带到期提醒字段的资产（域名注册到期、证书到期等）以全天条目落位日历「月 / 周 / 日」三种视图，图例标注「资产到期」；点击到期条目弹操作面板，可查看资产详情或一键加入系统日历（全天事件，桌面端落 .ics、移动端写系统日历）。
- 凭据展示行：资产详情的登录用户名/密码行支持掩码默认态、点击揭示（30 秒后自动隐藏）与一键复制。

### 修复（Fixed）

- 窄屏日历页头部溢出：标题、回到今天、月/周/日切换与翻页箭头挤在单行 Row，手机宽度（约 360dp）下溢出近 200px、「下一月」按钮被推出屏幕不可达；现按宽度降级为两行布局（第一行标题 + 翻页箭头、第二行视图切换 + 回到今天），宽屏保持单行不变。
- 周/日视图底部「日程与任务」列表条目不可点击：列表条目此前未绑定点击事件，现按条目类型分发（任务打开任务详情、资产到期弹操作面板、系统日程不响应），并补透明 Material 承载墨水涟漪消除 debug 断言告警。

## [0.2.2] - 2026-09-25

### 新增（Added）

- 日历系统集成与周/日视图：日历页新增「月 / 周 / 日」三种视图——周视图为 7 列 × 24 小时时间网格、日视图为单列 24 小时网格，任务与日程按时间落位（全天条目置顶、重叠条目自动分道），月视图保留原有月份网格与当天/本周/本月筛选；新增系统日历集成（移动端经 device_calendar 插件读写设备日历，需日历权限，Android 声明 READ/WRITE_CALENDAR；桌面端以 .ics iCalendar 文件落地到文档目录 Cardory/calendar/，可导入 Outlook/Apple 日历），月视图中的灰点表示系统日程，日历页提供「新建日程到系统日历」入口（标题/开始/结束/备注）。
...
- 桌面端与移动端页面过渡动画不统一：主题统一配置 Material 3 FadeForwardsPageTransitionsBuilder，全平台与移动端观感一致。

### 变更（Changed）

- 番茄钟新增自定义时长（1-600 分钟，输入校验），与预置专注/短休/长休并列；运行中会话的模式标签支持自定义显示。
