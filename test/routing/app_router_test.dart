...
void main() {
  testWidgets('门禁：未解锁时业务路由、占位路由与未知路径一律回到 /vault', (tester) async {
    final session = _SessionState(unlocked: false);
    addTearDown(session.dispose);
    final router = createAppRouter(
      vaultGateBuilder: (context) => const Scaffold(body: Text('门禁页')),
      workbenchBuilder: (context) => const Scaffold(body: Text('工作台')),
  ...
  testWidgets('解锁后可达工作台与受保护占位页；锁定后自动退回门禁页', (tester) async {
    final session = _SessionState(unlocked: false);
    addTearDown(session.dispose);
    final router = createAppRouter(
      vaultGateBuilder: (context) => const Scaffold(body: Text('门禁页')),
  ...
  testWidgets('解锁后未知路径收敛回工作台', (tester) async {
    final session = _SessionState(unlocked: true);
    addTearDown(session.dispose);
    final router = createAppRouter(
      vaultGateBuilder: (context) => const Scaffold(body: Text('门禁页')),
      workbenchBuilder: (context) => const Scaffold(body: Text('工作台')),
